...
export async function GET(request: Request) {
  try {
    ...
    if (shareError || !shareRecord) {
      return NextResponse.json({ error: 'Invalid or inactive share token' }, { status: 404 });
    }

    const { user_id: userId, entity_type: entityType, entity_id: entityId } = shareRecord;
    ...
    if (entityType === 'brain') {
      ...
      if (!doc || doc.status !== 'done') {
        return NextResponse.json({ error: 'Thought not found or still processing' }, { status: 404 });
      }

      // Important Security Check: Verify that the document belongs to the token owner
      if (!Array.isArray(doc.containerTags) || !doc.containerTags.includes(userId)) {
